...
{/literal}
	...
	<form method="post" id="fichaf_id" action="{geturl controller='contabilidad' action='fichafactura'}?id={if $smarty.post.id}{$smarty.post.id}{else}{$smarty.get.id}{/if}">
	    <div class="row_small_on" id="form_name_beg">
	    		<button class="submit2" type="submit" name="download" id="download" value="download">Descargar como PDF</button> 
	    </div>
	</form>
