...
{else}	
		...
		<tr>
			...
			<td class="links"><span>{if $fp->published != 'yes'}<a class="fancybox fancybox.iframe" href="{geturl controller='herramientas/facturas' action='editaritem'}?id={$item->getId()}&popup=true">{$item->profile->code|ucfirst}</a>{else}{$item->profile->code|ucfirst}{/if}</span></td>
