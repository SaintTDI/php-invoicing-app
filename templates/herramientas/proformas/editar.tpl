...
<div class="title" id="form_total_invoices">
	<label for="form_total_invoices"><h3>Editar Factura Pro-Forma:</h3></label>
</div>
<form method="post" id="prof_id" action="{geturl action='editar'}?id={$fp->proforma->getId()}">
    ...
    {* if $fp->proforma_consecutive}
    <div class="row" id="form_consecutive_">
        <label for="form_consecutive_">Consecutivo Pro-Forma:</label>
        <input type="text" class="stored" id="form_consecutive" name="proforma_consecutive" value="{$fp->proforma_consecutive}" placeholder="Ej: 282"/>
        {include file="lib/error.tpl" error=$fp->getError('proforma_consecutive')}
    </div>
    {else}
    {/if *}
    <input type="hidden" class="stored" id="form_consecutive" name="proforma_consecutive" value="" />
	...
	<div class="form_box">
		<div class="form_left">
		    <div class="row" id="form_proforma_number_">
		        ...
		        <input type="text" class="stored" id="form_proforma_number" name="proforma_number" value="{$fp->proforma_number}" onkeypress='validate(event)' placeholder="Ej: 999"/>
		...
		<div class="form_right">
		    <div class="row" id="form_proforma_date_">
		        ...
		        <input type="text" class="stored" id="form_proforma_date" name="proforma_date" value="{$fp->proforma_date}" placeholder="D&iacute;a-Mes-A&ntilde;o"/>
	...
	<div class="form_box">
	    <div class="row" id="form_invc_valid_">
	        ...
	        <select class="frecuency stored" id="form_proforma_vali" name="proforma_valid">
				<option value="1" {if $fp->proforma_valid == '1'} selected="selected" {/if}>1 d&iacute;a</option>
				<option value="7" {if $fp->proforma_valid == '7'} selected="selected" {/if}>7 d&iacute;as</option>
				<option value="15" {if $fp->proforma_valid == '15'} selected="selected" {/if}>15 d&iacute;as</option>
				<option value="30" {if $fp->proforma_valid == '30'} selected="selected" {/if}>30 d&iacute;as</option>
				<option value="45" {if $fp->proforma_valid == '45'} selected="selected" {/if}>45 d&iacute;as</option>
				<option value="60" {if $fp->proforma_valid == '60'} selected="selected" {/if}>60 d&iacute;as</option>
			</select>
    ...
    <div class="row" id="form_proforma_frequency">
        ...
        <select class="frecuency stored" id="form_proforma_frequency" name="proforma_frequency">
			<option value="una vez" {if $fp->proforma_frequency == 'una vez'} selected="selected" {/if}>Una vez</option>
			<option value="semanal" {if $fp->proforma_frequency == 'semanal'} selected="selected" {/if}>Semanal</option>
			<option value="quincenal" {if $fp->proforma_frequency == 'quincenal'} selected="selected" {/if}>Quincenal</option>
			<option value="mensual" {if $fp->proforma_frequency == 'mensual'} selected="selected" {/if}>Mensual</option>
			<option value="bimensual" {if $fp->proforma_frequency == 'bimensual'} selected="selected" {/if}>Bimensual</option>
			<option value="trimestral" {if $fp->proforma_frequency == 'trimestral'} selected="selected" {/if}>Trimestral</option>
			<option value="semestral" {if $fp->proforma_frequency == 'semestral'} selected="selected" {/if}>Semestral</option>
		</select>
    ...
    *}
    ...
    {if $company|@count == 0}
         <label for="form_company_ids">Cliente:</label>
    		<a class="fancybox fancybox.iframe submit2" href="{geturl controller='cproforma' action='agregarcompania'}?document_id={$fp->proforma->getId()}">Agregar Cliente</a>
	...
	<tr>
	  ...
	  <td class="links"><span><a class="fancybox fancybox.iframe" href="{geturl controller='cproforma' action='editarcompania'}?id={$comp->company_id}&document_id={$fp->proforma->getId()}">
  ...
  {/foreach}
		...
		<div class="form_left">
		 	<div class="row" id="form_contact_container">
		        <label for="form_contact_ids">Contacto:</label>
		        	    <input type="text" class="stored" id="form_contact" name="proforma_contact" value="{$fp->proforma_contact}" placeholder="Nombre Apellido"/>
		...
		<div class="form_right">
		 	<div class="row" id="form_project_container">
		        <label for="form_project_ids">Proyecto:</label>
		        	    <input type="text" class="stored" id="form_project" name="proforma_project" value="{$fp->proforma_project}" placeholder="Ej: Proyecto Vi&ntilde;edo"/>
    ...
    {if $items|@count == 0}
    		...
    		<a class="fancybox fancybox.iframe submit2" href="{geturl action='agregaritem'}?document_type=proforma&document_id={$fp->proforma->getId()}">Agregar Nuevo Item</a>
    		{include file='herramientas/proformas/item.tpl' document_type='proforma' document_id=$fp->proforma->getId()}
    ...
    <div class="row" id="form_proforma_copy_">
    		...
    		<input type="checkbox" class="stored" id="form_proforma_copy" name="proforma_copy" value="true" {if $fp->proforma_copy}checked="checked"{/if}> 
	</div>
	*}
	<div class="row" id="form_proforma_terms">
    		<label for="form_proforma_terms">T&eacute;rminos:</label>
    		<textarea id="form_proforma_terms" class="big_textarea stored" name="terms" rows="6" cols="50" placeholder="Coloca aqu&iacute; cualquier condici&oacute;n de pago o mensaje que desees que aparezca en la Factura Pro-Forma.">{$fp->terms}</textarea>
	...
	<div class="row" id="form_proforma_notes">
    		...
    		<textarea id="form_proforma_notes" class="big_textarea stored" name="notes" rows="6" cols="50" placeholder="Coloca aqu&iacute; cualquier nota o comentario personal sobre este documento. Lo que escribas s&oacute;lo ser&aacute; visible para los usuarios de tu cuenta.">{$fp->notes}</textarea>
		...
		<div class="row">
	    		...
	    		{* <span><a class="fancybox fancybox.iframe submit2" href="{geturl controller='herramientas/proformas' action='preview'}?id={$fp->proforma->getId()}&popup=true">Enviar/Descargar</a></span>
