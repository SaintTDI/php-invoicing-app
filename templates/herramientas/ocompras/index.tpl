...
<div class="boxes3">
	...
	<div class="boton_top">
	    <label for="bt_">
			<a class="submit6" {if $identity->trial_expired}{else}href="{geturl controller='herramientas/ocompras' action='crear'}"{/if} onclick="clearLocalStorage();">Nueva O/ Compra</a>
	 	</label>
	</div>
</div>
...
{else}
		<div class="title" id="form_total_invoices">
    	   ...
    	   <form method="post" id="purchas_id" action="{geturl action='index'}">
    	    <table class="table_p">
    	  	 <tr>
         ...
         {assign var='iva_' value=0}{assign var='iva2_' value=0}{assign var='i' value=0}{foreach from=$purchases item=purchase}
			<tr>
				<td class="links"><span><a href="{geturl controller='herramientas/ocompras' action='fichaocompra'}?id={$purchase->getId()}" onclick="clearLocalStorage();">{if $purchase->profile->start_letter}{$purchase->profile->start_letter} -{/if} {$purchase->profile->number_zero}{$purchase->purchase_number}</a></span></td>
				<td>{$purchase->profile->purchase_date}</td>
				<td>{$purchase->profile->thecompany|ucfirst}</td>
				<td>{$purchase->profile->total|number_format:2:',':'.'}</td>
        ...
        {assign var='i' value=$i+1}{assign var='iva_' value=0}{assign var='iva2_' value=0}{/foreach}
    	  	...
    	  	{* <td><input type="checkbox" name="purchase_id[]" value="{$purchase->getId()}" {if $purchase->profile->published == 'yes'}disabled="disabled"{/if}></td> *}
    	  	{* {foreach from=$totalIva key='key' item='label'}{if $label == 'I.V.A.'}{assign var='iva_' value=$iva_ + $purchase->profile->$key}{/if}{/foreach}{foreach from=$totalIva key='key' item='label'}{if $label == 'Otros Imp.'}{assign var='iva2_' value=$iva2_ + $purchase->profile->$key}{/if}{/foreach} *}
    	  	 	<tr>
