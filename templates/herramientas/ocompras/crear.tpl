...
<div class="title" id="form_total_invoices">
	<label for="form_total_invoices"><h3>Nueva &Oacute;rden de Compra:</h3></label>
</div>
<form method="post" id="purchas_id" action="{geturl action='crear'}?id={$fp->purchase->getId()}">
    ...
    <script type="text/javascript">	
      jQuery(document).ready(function() {
   		var r = confirm('Por favor completa el perfil de tu Empresa antes de crear una Orden de Compra.');
        if (r) {
   ...
   {/if}
    ...
    <div class="row" id="form_consecutive_">
        ...
        <input type="text" class="stored" id="form_consecutive" name="purchase_consecutive" value="{$default_consecutive + $purchasecount}" placeholder="Ej: 282"/>
	...
	<div class="form_box">
		<div class="form_left">
		    <div class="row" id="form_purchase_number_">
		        ...
		        <input type="text" class="stored" id="form_purchase_number" name="purchase_number" value="{$purchasecount + 1}" onkeypress='validate(event)' placeholder="Ej: 999"/>
		    ...
		    <div class="row" id="form_invc_valid_">
		        ...
		        <select class="frecuency stored" id="form_purchase_vali" name="purchase_valid">
					<option value="1" {if $smarty.post.purchase_valid == '1'} selected="selected" {/if}>1 d&iacute;a</option>
					<option value="7" {if $smarty.post.purchase_valid == '7'} selected="selected" {/if}>7 d&iacute;as</option>
					<option value="15" {if $smarty.post.purchase_valid == '15'} selected="selected" {/if}>15 d&iacute;as</option>
					<option value="30" {if $smarty.post.purchase_valid == '30'} selected="selected" {/if}>30 d&iacute;as</option>
					<option value="45" {if $smarty.post.purchase_valid == '45'} selected="selected" {/if}>45 d&iacute;as</option>
					<option value="60" {if $smarty.post.purchase_valid == '60'} selected="selected" {/if}>60 d&iacute;as</option>
				</select>
		...
		<div class="form_right">
		    <div class="row" id="form_purchase_date_">
		        ...
		        <input type="text" class="stored" id="form_purchase_date" name="purchase_date" value="{if $smarty.post.purchase_date}{$smarty.post.purchase_date}{else}{$ts_date}{/if}" placeholder="D&iacute;a-Mes-A&ntilde;o"/>
		    ...
		    <div class="row" id="form_purchase_frequency">
		        ...
		        <select class="frecuency stored" id="form_purchase_frequency" name="purchase_frequency">
					<option value="una vez" {if $smarty.post.purchase_frequency == 'una vez'} selected="selected" {/if}>Una vez</option>
					<option value="semanal" {if $smarty.post.purchase_frequency == 'semanal'} selected="selected" {/if}>Semanal</option>
					<option value="quincenal" {if $smarty.post.purchase_frequency == 'quincenal'} selected="selected" {/if}>Quincenal</option>
					<option value="mensual" {if $smarty.post.purchase_frequency == 'mensual'} selected="selected" {/if}>Mensual</option>
					<option value="bimensual" {if $smarty.post.purchase_frequency == 'bimensual'} selected="selected" {/if}>Bimensual</option>
					<option value="trimestral" {if $smarty.post.purchase_frequency == 'trimestral'} selected="selected" {/if}>Trimestral</option>
					<option value="semestral" {if $smarty.post.purchase_frequency == 'semestral'} selected="selected" {/if}>Semestral</option>
				</select>
 	...
 	<div class="form_box">
	 	<div class="row" id="form_budget_container">
	        <label for="form_contact_ids">Presupuesto N&ordm;:</label>
	        	<input type="text" class="stored" id="form_budget_number" name="budget_number" value="{$smarty.post.budget_number}" placeholder="Ej: A201"/>
	  	</div>
  	</div>
    ...
    {if $company|@count == 0}
         <label for="form_company_ids">Proveedor <strong>&#x28;&#x2A;&#x29;</strong>:</label>
    		<a class="fancybox fancybox.iframe submit2" href="{geturl controller='cpurchase' action='agregarcompania'}?document_id={$inv_id}">Agregar Proveedor</a>
   ...
   {if $counter == 0}{assign var='counter' value=$counter +1}
	...
	<tr>
	  ...
	  <td class="links"><span><a class="fancybox fancybox.iframe" href="{geturl controller='cpurchase' action='editarcompania'}?id={$comp->company_id}&document_id={$inv_id}">
   ...
   {/foreach}
		...
		<div class="form_left">
		 	<div class="row" id="form_contact_container">
		        <label for="form_contact_ids">Contacto:</label>
		        	    <input type="text" class="stored" id="form_contact" name="purchase_contact" value="{$smarty.post.purchase_contact}" placeholder="Nombre Apellido"/>
		...
		<div class="form_right">
		 	<div class="row" id="form_project_container">
		        <label for="form_project_ids">Proyecto:</label>
		        	    <input type="text" class="stored" id="form_project" name="purchase_project" value="{$smarty.post.purchase_project}" placeholder="Ej: Proyecto Vi&ntilde;edo"/>
    ...
    {if $items|@count == 0}
    		...
    		<a class="fancybox fancybox.iframe submit2" href="{geturl action='agregaritem'}?document_type=purchase&document_id={$inv_id}">Agregar Nuevo Item</a>
    		{include file='herramientas/ocompras/item.tpl' document_type='purchase' document_id=$inv_id}
    ...
    {* <div class="row" id="form_recc_">
    		...
    		<input type="checkbox" class="stored" id="form_recc" name="recc" value="true" {if $default_recc_ || $smarty.post.recc}checked="checked"{/if}> 
	</div> 

    <div class="row" id="form_purchase_copy_">
    		<span for="form_purchase_copy_">Copia de O/C:</span>
    		<input type="checkbox" class="stored" id="form_purchase_copy" name="purchase_copy" value="true" {if $smarty.post.purchase_copy}checked="checked"{/if}> 
	</div> *}
	
	<input type="hidden" id="form_recc" name="recc" value="{$default_recc_}" />
	
	<div class="row" id="form_purchase_terms">
    		<label for="form_purchase_terms">T&eacute;rminos:</label>
    		<textarea id="form_purchase_terms" class="big_textarea stored" name="terms" rows="6" cols="50" placeholder="Coloca aqu&iacute; cualquier condici&oacute;n de pago o mensaje que desees que aparezca en la Orden de Compra.">{$smarty.post.terms}</textarea>
	...
	<div class="row" id="form_purchase_notes">
    		...
    		<textarea id="form_purchase_notes" class="big_textarea stored" name="notes" rows="6" cols="50" placeholder="Coloca aqu&iacute; cualquier nota o comentario personal sobre este documento. Lo que escribas s&oacute;lo ser&aacute; visible para los usuarios de tu cuenta.">{$smarty.post.notes}</textarea>
