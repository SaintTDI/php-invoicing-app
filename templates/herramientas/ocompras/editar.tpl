...
<div class="title" id="form_total_invoices">
	<label for="form_total_invoices"><h3>Editar &Oacute;rden de Compra:</h3></label>
</div>

<form method="post" id="purchas_id" action="{geturl action='editar'}?id={$fp->purchase->getId()}">
    ...
    {* if $fp->purchase_consecutive}
    <div class="row" id="form_consecutive_">
        <label for="form_consecutive_">Consecutivo O/C:</label>
        <input type="text" class="stored" id="form_consecutive" name="purchase_consecutive" value="{$fp->purchase_consecutive}" placeholder="Ej: 282"/>
        {include file="lib/error.tpl" error=$fp->getError('purchase_consecutive')}
    </div>
    {else}
    {/if *}
    <input type="hidden" class="stored" id="form_consecutive" name="purchase_consecutive" value="" />
    <input type="hidden" class="stored" id="form_currency" name="currency" value="{$fp->currency}">

	<div class="form_box">
		<div class="form_left">
		    <div class="row" id="form_purchase_number_">
		        <label for="form_purchase_number_">N&ordm; de O/C <strong>&#x28;&#x2A;&#x29;</strong>:</label>
		        <input type="text" class="stored" id="form_purchase_number" name="purchase_number" value="{$fp->purchase_number}" onkeypress='validate(event)'  placeholder="Ej: 999"/>
		        {include file="lib/error.tpl" error=$fp->getError('purchase_number')}
		    </div>
		    <div class="row" id="form_invc_valid_">
		    	<label for="form_invc_vali">Validez:</label>
		        <select class="frecuency stored" id="form_purchase_vali" name="purchase_valid">
					<option value="1" {if $fp->purchase_valid == '1'} selected="selected" {/if}>1 d&iacute;a</option>
					<option value="7" {if $fp->purchase_valid == '7'} selected="selected" {/if}>7 d&iacute;as</option>
					<option value="15" {if $fp->purchase_valid == '15'} selected="selected" {/if}>15 d&iacute;as</option>
					<option value="30" {if $fp->purchase_valid == '30'} selected="selected" {/if}>30 d&iacute;as</option>
					<option value="45" {if $fp->purchase_valid == '45'} selected="selected" {/if}>45 d&iacute;as</option>
					<option value="60" {if $fp->purchase_valid == '60'} selected="selected" {/if}>60 d&iacute;as</option>
				</select>
		        {* include file="lib/error.tpl" error=$fp->getError('purchase_valid') *}
		    </div>
		</div>
		<div class="form_right">
		    <div class="row" id="form_purchase_date_">
		        <label for="form_purchase_date_">Fecha <strong>&#x28;&#x2A;&#x29;</strong>:</label>
		        <input type="text" class="stored" id="form_purchase_date" name="purchase_date" value="{$fp->purchase_date}" placeholder="D&iacute;a-Mes-A&ntilde;o"/>
		        {include file="lib/error.tpl" error=$fp->getError('purchase_date')}
		    </div>
		    <div class="row" id="form_purchase_frequency">
		    	<label for="form_purchase_frequency">Frecuencia:</label>
		        <select class="frecuency stored" id="form_purchase_frequency" name="purchase_frequency">
					<option value="una vez" {if $fp->purchase_frequency == 'una vez'} selected="selected" {/if}>Una vez</option>
					<option value="semanal" {if $fp->purchase_frequency == 'semanal'} selected="selected" {/if}>Semanal</option>
					<option value="quincenal" {if $fp->purchase_frequency == 'quincenal'} selected="selected" {/if}>Quincenal</option>
					<option value="mensual" {if $fp->purchase_frequency == 'mensual'} selected="selected" {/if}>Mensual</option>
					<option value="bimensual" {if $fp->purchase_frequency == 'bimensual'} selected="selected" {/if}>Bimensual</option>
					<option value="trimestral" {if $fp->purchase_frequency == 'trimestral'} selected="selected" {/if}>Trimestral</option>
					<option value="semestral" {if $fp->purchase_frequency == 'semestral'} selected="selected" {/if}>Semestral</option>
				</select>
		        {include file="lib/error.tpl" error=$fp->getError('purchase_frequency')}
		    </div>
		</div>
	</div>
 
	<div class="form_box">
	 	<div class="row" id="form_budget_container">
	        <label for="form_contact_ids">Presupuesto N&ordm;:</label>
	        	<input type="text" class="stored" id="form_budget_number" name="budget_number" value="{$fp->budget_number}" placeholder="Ej: A201"/>
	  	</div>
  	</div>
    ...
    {if $company|@count == 0}
         <label for="form_company_ids">Proveedor <strong>&#x28;&#x2A;&#x29;</strong>:</label>
    		<a class="fancybox fancybox.iframe submit2" href="{geturl controller='cpurchase' action='agregarcompania'}?document_id={$fp->purchase->getId()}">Agregar Proveedor</a>
	...
	<tr>
	  ...
	  <td class="links"><span>{if $fp->published != 'yes'}<a class="fancybox fancybox.iframe" href="{geturl controller='cpurchase' action='editarcompania'}?id={$comp->company_id}&document_id={$fp->purchase->getId()}">
  ...
  {/foreach}
		...
		<div class="form_left">
		 	<div class="row" id="form_contact_container">
		        <label for="form_contact_ids">Contacto:</label>
		        	    <input type="text" class="stored" id="form_contact" name="purchase_contact" value="{$fp->purchase_contact}" placeholder="Nombre Apellido"/>
		...
		<div class="form_right">
		 	<div class="row" id="form_project_container">
		        <label for="form_project_ids">Proyecto:</label>
		        	    <input type="text" class="stored" id="form_project" name="purchase_project" value="{$fp->purchase_project}" placeholder="Ej: Proyecto Vi&ntilde;edo"/>
    ...
    {if $items|@count == 0}
    		...
    		<a class="fancybox fancybox.iframe submit2" href="{geturl action='agregaritem'}?document_type=purchase&document_id={$fp->purchase->getId()}">Agregar Nuevo Item</a>
    		{include file='herramientas/ocompras/item.tpl' document_type='purchase' document_id=$fp->purchase->getId()}
	    	<input type="hidden" id="form_add_check2" name="add_check2" value="false">
	    	{include file="lib/error.tpl" error=$fp->getError('add_check2')}
    {else}
    		<label for="form_item">Item(s) <strong>&#x28;&#x2A;&#x29;</strong>:</label>	
    		{include file='herramientas/ocompras/item.tpl' document_type='purchase' document_id=$fp->purchase->getId()}
    		{if $fp->published != 'yes'}<a class="fancybox fancybox.iframe submit2" href="{geturl action='agregaritem'}?document_type=purchase&document_id={$fp->purchase->getId()}">Agregar Nuevo Item</a>{/if}
	...
	{include file='herramientas/ocompras/taxes.tpl'}
    ...
    {* <div class="row" id="form_recc_">
    		<span for="form_recc_">Regimen especial (RECC):</span>
    		<input type="checkbox" class="stored" id="form_recc" name="recc" value="true" {if $fp->recc}checked="checked"{elseif $default_recc_}checked="checked"{/if}> 
	</div> 

    <div class="row" id="form_purchase_copy_">
    		<label for="form_purchase_copy_">Copia de O/C:</label>
    		<input type="checkbox" class="stored" id="form_purchase_copy" name="purchase_copy" value="true" {if $fp->purchase_copy}checked="checked"{/if}> 
	</div> *}
	
	<input type="hidden" id="form_recc" name="recc" value="{$fp->recc}" />
	
	<div class="row" id="form_purchase_terms">
    		<label for="form_purchase_terms">T&eacute;rminos:</label>
    		<textarea id="form_purchase_terms" class="big_textarea stored" name="terms" rows="6" cols="50" placeholder="Coloca aqu&iacute; cualquier condici&oacute;n de pago o mensaje que desees que aparezca en la Orden de Compra.">{$fp->terms}</textarea>
    		{include file="lib/error.tpl" error=$fp->getError('terms')}
    	</div>
    	
	<div class="row" id="form_purchase_notes">
    		<label for="form_purchase_notes">Notas personales:</label>
    		<textarea id="form_purchase_notes" class="big_textarea stored" name="notes" rows="6" cols="50" placeholder="Coloca aqu&iacute; cualquier nota o comentario personal sobre este documento. Lo que escribas s&oacute;lo ser&aacute; visible para los usuarios de tu cuenta.">{$fp->notes}</textarea>
    		{include file="lib/error.tpl" error=$fp->getError('notes')}
    	</div>
    	
    	{if $fp->published == 'yes'}
	<div class="row center">
    		<span><a class="fancybox fancybox.iframe submit2" href="{geturl controller='herramientas/ocompras' action='preview'}?id={$fp->purchase->getId()}&popup=true">Enviar/Descargar</a></span>
	</div>
	{else}
	<div class="form_box">
		<div class="row">
			<label for="form_source">&nbsp;</label>
	    		...
	    		{* <span><a class="fancybox fancybox.iframe submit2" href="{geturl controller='herramientas/ocompras' action='preview'}?id={$fp->purchase->getId()}&popup=true">Enviar</a></span>
			{if $fp->transformed != 'yes'}<button class="submit" type="submit" name="convert" id="convert" value="convert" onclick="return confirmDone3()">Transformar a Nota de Gasto</button>{/if *}
		</div>
	</div>
	{/if}
	<div class="form_box">
