...
{else}	
		...
		<tr>
			<td class="table_input"><input type="hidden" id="form_product_id" name="product_id[]" value="{$item->profile->product_id}" /><input type="hidden" id="form_item_id2" name="item_id2[]" value="{$item->getId()}"><input type="checkbox" id="form_item_id" name="item_id[]" value="{$item->getId()}"></td>
			<td class="links"><span><a class="fancybox fancybox.iframe" href="{geturl controller='herramientas/presupuestos' action='editaritem'}?id={$item->getId()}&popup=true">{$item->profile->code|ucfirst}</a></span></td>
		...
		{assign var='iva_total__' scope=global value=$item->profile->quantity * $item->profile->unit_price}
		{assign var='subtotal' scope=global value=$subtotal + $iva_total__}
		{assign var='iva_total_r1' scope=global value=$iva_total_r1 + $item->profile->iva_total}
		{assign var='iva2_total_r1' scope=global value=$iva2_total_r1 + $item->profile->iva2_total}
