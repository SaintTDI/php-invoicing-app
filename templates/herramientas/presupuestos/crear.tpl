...
<form method="post" id="budget__id" action="{geturl action='crear'}?id={$fp->budget->getId()}">
   ...
   {* if $default_consecutive}
    <div class="row" id="form_consecutive_">
        ...
        <input type="text" class="stored" id="form_consecutive" name="budget_consecutive" value="{$default_consecutive + $budgetcount}" placeholder="Ej: 282"/>
        {include file="lib/error.tpl" error=$fp->getError('budget_consecutive')}
    </div>
    {else}
    {/if *}
    <input type="hidden" class="stored" id="form_consecutive" name="budget_consecutive" value="{$smarty.post.budget_consecutive}" />
	...
	<div class="form_box">
		<div class="form_left">
		    <div class="row" id="form_budget_number_">
		        ...
		        <input type="text" class="stored" id="form_budget_number" name="budget_number" value="{if $smarty.post.budget_number}{$smarty.post.budget_number}{else}{$budgetcount + 1}{/if}" onkeypress='validate(event)' placeholder="Ej: 999"/>
		...
		<div class="form_right">
		    <div class="row" id="form_budget_date_">
		        ...
		        <input type="text" class="stored" id="form_budget_date" name="budget_date" value="{if $smarty.post.budget_date}{$smarty.post.budget_date}{else}{$ts_date}{/if}" placeholder="D&iacute;a-Mes-A&ntilde;o"/>
	...
	<div class="form_box">
	    <div class="row" id="form_invc_valid_">
	        ...
	        <select class="frecuency stored" id="form_budget_vali" name="budget_valid">
				<option value="1" {if $smarty.post.budget_valid == '1'} selected="selected" {/if}>1 d&iacute;a</option>
				<option value="7" {if $smarty.post.budget_valid == '7'} selected="selected" {/if}>7 d&iacute;as</option>
				<option value="15" {if $smarty.post.budget_valid == '15'} selected="selected" {/if}>15 d&iacute;as</option>
				<option value="30" {if $smarty.post.budget_valid == '30'} selected="selected" {/if}>30 d&iacute;as</option>
				<option value="45" {if $smarty.post.budget_valid == '45'} selected="selected" {/if}>45 d&iacute;as</option>
				<option value="60" {if $smarty.post.budget_valid == '60'} selected="selected" {/if}>60 d&iacute;as</option>
			</select>
    ...
    <div class="row" id="form_budget_frequency">
        ...
        <select class="frecuency stored" id="form_budget_frequency" name="budget_frequency">
			<option value="una vez" {if $smarty.post.budget_frequency == 'una vez'} selected="selected" {/if}>Una vez</option>
			<option value="semanal" {if $smarty.post.budget_frequency == 'semanal'} selected="selected" {/if}>Semanal</option>
			<option value="quincenal" {if $smarty.post.budget_frequency == 'quincenal'} selected="selected" {/if}>Quincenal</option>
			<option value="mensual" {if $smarty.post.budget_frequency == 'mensual'} selected="selected" {/if}>Mensual</option>
			<option value="bimensual" {if $smarty.post.budget_frequency == 'bimensual'} selected="selected" {/if}>Bimensual</option>
			<option value="trimestral" {if $smarty.post.budget_frequency == 'trimestral'} selected="selected" {/if}>Trimestral</option>
			<option value="semestral" {if $smarty.post.budget_frequency == 'semestral'} selected="selected" {/if}>Semestral</option>
		</select>
    ...
    *}
    ...
    {if $company|@count == 0}
         <label for="form_company_ids">Cliente <strong>&#x28;&#x2A;&#x29;</strong>:</label>
    		<a class="fancybox fancybox.iframe submit2" href="{geturl controller='cbudget' action='agregarcompania'}?document_id={$inv_id}">Agregar Cliente</a>
   ...
   {if $counter == 0}{assign var='counter' value=$counter +1}
	...
	<tr>
	  ...
	  <td class="links"><span><a class="fancybox fancybox.iframe" href="{geturl controller='cbudget' action='editarcompania'}?id={$comp->company_id}&document_id={$inv_id}">
   ...
   {/foreach}
		...
		<div class="form_left">
		 	<div class="row" id="form_contact_container">
		        <label for="form_contact_ids">Contacto:</label>
		        	    <input type="text" class="stored" id="form_contact" name="budget_contact" value="{$smarty.post.budget_contact}" placeholder="Nombre Apellido"/>
		...
		<div class="form_right">
		 	<div class="row" id="form_project_container">
		        <label for="form_project_ids">Proyecto:</label>
		        	    <input type="text" class="stored" id="form_project" name="budget_project" value="{$smarty.post.budget_project}" placeholder="Ej: Proyecto Vi&ntilde;edo"/>
    ...
    {if $items|@count == 0}
    		...
    		<a class="fancybox fancybox.iframe submit2" href="{geturl action='agregaritem'}?document_type=budget&document_id={$inv_id}">Agregar Nuevo Item</a>
    		{include file='herramientas/presupuestos/item.tpl' document_type='budget' document_id=$inv_id}
    ...
    <div class="row" id="form_budget_copy_">
    		...
    		<input type="checkbox" class="stored" id="form_budget_copy" name="budget_copy" value="true" {if $smarty.post.budget_copy}checked="checked"{/if}> 
	</div>
	*}
	<div class="row" id="form_budget_terms">
    		<label for="form_budget_terms">T&eacute;rminos:</label>
    		<textarea id="form_budget_terms" class="big_textarea stored" name="terms" rows="6" cols="50" placeholder="Coloca aqu&iacute; cualquier condici&oacute;n de pago o mensaje que desees que aparezca en el presupuesto.">{$smarty.post.terms}</textarea>
	...
	<div class="row" id="form_budget_notes">
    		...
    		<textarea id="form_budget_notes" class="big_textarea stored" name="notes" rows="6" cols="50" placeholder="Coloca aqu&iacute; cualquier nota o comentario personal sobre este documento. Lo que escribas s&oacute;lo ser&aacute; visible para los usuarios de tu cuenta.">{$smarty.post.notes}</textarea>
