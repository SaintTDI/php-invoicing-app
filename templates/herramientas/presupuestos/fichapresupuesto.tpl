...
<div class="title" id="form_total_invoices">
	<label for="form_total_invoices"><h3>Ficha de Presupuesto:</h3></label>
</div>
{literal}
...
{/literal}
	...
	<form method="post" id="budget__id" action="{geturl controller='herramientas/presupuestos' action='fichapresupuesto'}?id={if $smarty.post.id}{$smarty.post.id}{else}{$smarty.get.id}{/if}">
	    <div class="row_small_on" id="form_name_beg"> 
	 	    {if $budget->profile->published != 'yes'}<label for="form_budget_link"><a class="submit8" {if $identity->trial_expired}{else}href="{geturl controller='herramientas/presupuestos' action='editar'}?id={$budget->getId()}"{/if}>Editar</a></label>{/if}
   
      		<button class="submit8" type="button" name="send" id="send" value="send" onclick="showDiv()">Enviar...</button> 

	    		<button class="submit8" type="submit" name="download" id="download" value="download" {if !isset($budget->profile->budget_file)}onclick="return confirmDone()"{/if}>Descargar como PDF</button> 

			{if $identity->trial_expired}{else}{if $budget->profile->transformed != 'yes'}<button class="submit8" type="submit" name="convert" id="convert" value="convert" onclick="return confirmDone2()">Facturar</button>{/if}{/if}
	   
			{if $budget->profile->transformed != 'yes'}{if $budget->profile->published != 'yes'}<button class="submit8" type="submit" name="delete" id="delete" value="delete" onclick="return confirmDone5()">Borrar Documento</button>{/if}{/if}
	    </div>
	</form>
	
	
	<form id="budget__id2" method="post" action="{geturl action='fichapresupuesto'}?id={$smarty.get.id}&submitted=true">
	    {if $fp->hasError()}
	    ...
	    {/if} 
	        ...
	        <input class="social" type="email" id="form_email" name="email" value="{$smarty.post.email}" placeholder="Ej: ejemplo@webproadmin.com"/>
	    		<button class="submit7" type="submit" name="send" id="send" value="send" {if !isset($budget->profile->budget_file)}onclick="return confirmDone()"{/if}>Enviar</button> 
		    </div>
		    <div class="row_small2" id="form_name_send">
    ...
    {if $budget->profile->terms}
    <div class="conditions_row" id="form_name_">
			<label for="form_budget_company_">Condiciones:</label>
			<span>{$budget->profile->terms}</span>
    </div>
	{/if}
    {if $budget->profile->notes}
    <div class="conditions_row" id="form_name_">
			<label for="form_budget_company_">Notas Personales:</label>
			<span>{$budget->profile->notes}</span>
    </div>
	{/if}
