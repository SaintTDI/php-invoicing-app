...
<div class="boxes3">
	...
	<div class="boton_top">
	    <label for="bt_">
			<a class="submit6" {if $identity->trial_expired}{else}href="{geturl controller='herramientas/presupuestos' action='crear'}"{/if} onclick="clearLocalStorage();">Nuevo Presupuesto</a>
	 	</label>
	</div>
</div>
		{if $budgets|@count == 0}
...
{else}
	   <div class="title" id="form_total_invoices">
    	   ...
    	   <form method="post" id="budget__id" action="{geturl action='index'}">
    	    <table class="table_p">
    	  	 <tr>
         ...
         {assign var='iva_' value=0}{assign var='iva2_' value=0}{assign var='i' value=0}{foreach from=$budgets item=budget}
			<tr>
				<td class="links"><span><a href="{geturl controller='herramientas/presupuestos' action='fichapresupuesto'}?id={$budget->getId()}" onclick="clearLocalStorage();">{if $budget->profile->start_letter}{$budget->profile->start_letter} -{/if} {$budget->profile->number_zero}{$budget->budget_number}</a></span></td>
				<td>{$budget->profile->budget_date}</td>
				<td>{$budget->profile->thecompany|ucfirst}</td>
				<td>{$budget->profile->total|number_format:2:',':'.'}</td>
        ...
        {* <td><input type="checkbox" name="budget_id[]" value="{$budget->getId()}"></td> *}
        {* {foreach from=$totalIva key='key' item='label'}{if $label == 'I.V.A.'}{assign var='iva_' value=$iva_ + $budget->profile->$key}{/if}{/foreach}{foreach from=$totalIva key='key' item='label'}{if $label == 'Otros Imp.'}{assign var='iva2_' value=$iva2_ + $budget->profile->$key}{/if}{/foreach} *}
    	  	 	<tr>
