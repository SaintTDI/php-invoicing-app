...
<div class="title" id="form_total_invoices">
	<label for="form_total_invoices"><h3>Editar Advisor:</h3></label>
</div>
<form method="post" id="adv_id" action="{geturl action='editar'}?id={$fp->advisor->getId()}" enctype="multipart/form-data">
    {if $fp->hasError()}
        		<div class="error">
    ...
    <div class="row" id="form_contact_last_name">
 	...
 	<div class="row" id="form_company_container">
        <label for="form_company_ids">Empresa:</label>
        	    <input type="text" id="form_company" name="company" value="{if $fp->company_id}{assign var='id' value=$fp->company_id}{assign var='b' value=0}{foreach from=$companieslist item=company}{if $company_id_[$b] == $id}{$company_[$b]|ucfirst}{break}{/if}{assign var='b' value=$b+1}{/foreach}{elseif $fp->thecompany && !$fp->company_id}{$fp->thecompany|ucfirst}{/if}" placeholder="Empresa, Sociedad o Persona"/>
        		<input type="hidden" id="form_company_id" name="company_id" value="{assign var='id' value=$fp->company_id}{assign var='b' value=0}{foreach from=$companieslist item=company}{if $company_id_[$b] == $id}{$company_id_[$b]}{break}{/if}{assign var='b' value=$b+1}{/foreach}" />
        		<input type="hidden" id="form_address_id" name="address_id" value="{assign var='id' value=$fp->company_id}{assign var='b' value=0}{foreach from=$companieslist item=company}{if $company_id_[$b] == $id}{if $address_id_[$b]|is_array}{foreach from=$address_id_[$b] item=address}{$address}{break}{/foreach}{else}{$address_id_[$b]}{break}{/if}{/if}{assign var='b' value=$b+1}{/foreach}" />
	...
	{literal}<script type="text/javascript">
 	...
 	<div class="row" id="form_contact_notes">
    		...
    		<textarea id="form_contact_notes" name="notes" rows="6" cols="50" placeholder="Coloca aqu&iacute; cualquier nota o comentario personal sobre este Advisor. Lo que escribas s&oacute;lo ser&aacute; visible para los usuarios de tu cuenta.">{$fp->notes}</textarea>
