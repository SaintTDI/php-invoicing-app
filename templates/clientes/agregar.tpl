...
<div class="title" id="form_total_invoices">
	<label for="form_total_invoices"><h3>Nuevo Cliente:</h3></label>
</div>
<form method="post" id="client_id_" action="{geturl action='agregar'}?id={$fp->client->getId()}" enctype="multipart/form-data">
    ...
    <div class="row" id="form_contact_last_name">
 	...
 	<div class="row" id="form_company_container">
        <label for="form_company_ids">Empresa:</label>
        	    <input type="text" id="form_company" name="company" value="{if $smarty.post.company}{$smarty.post.company}{else}{if $fp->company_id}{assign var='id' value=$fp->company_id}{assign var='b' value=0}{foreach from=$companieslist item=company}{if $company_id_[$b] == $id}{$company_[$b]|ucfirst}{break}{/if}{assign var='b' value=$b+1}{/foreach}{elseif $fp->thecompany && !$fp->company_id}{$fp->thecompany|ucfirst}{/if}{/if}" placeholder="Empresa, sociedad o persona"/>
        		<input type="hidden" id="form_company_id" name="company_id" value="{if $smarty.post.company_id}{$smarty.post.company_id}{else}{assign var='id' value=$fp->company_id}{assign var='b' value=0}{foreach from=$companieslist item=company}{if $company_id_[$b] == $id}{$company_id_[$b]}{break}{/if}{assign var='b' value=$b+1}{/foreach}{/if}" />
        		<input type="hidden" id="form_address_id" name="address_id" value="{if $smarty.post.address_id}{$smarty.post.address_id}{else}{assign var='id' value=$fp->company_id}{assign var='b' value=0}{foreach from=$companieslist item=company}{if $company_id_[$b] == $id}{if $address_id_[$b]|is_array}{foreach from=$address_id_[$b] item=address}{$address}{break}{/foreach}{else}{$address_id_[$b]}{break}{/if}{/if}{assign var='b' value=$b+1}{/foreach}{/if}" />
    ...
    <div class="row" id="form_mobile_container">
        ...
        <input class="phone" type="text" id="form_mobile_country" name="mobile_country" value="{$smarty.post.mobile_country}" onkeypress='validate(event)' />
        <input class="phone" type="text" id="form_mobile_area" name="mobile_area" value="{$smarty.post.mobile_area}" onkeypress='validate(event)' />
        <input type="text" id="form_mobile" name="mobile" value="{$smarty.post.mobile}" onkeypress='validate(event)' />
    ...
    <div class="row" id="form_phone_container">
        ...
        <input class="phone" type="text" id="form_phone_country" name="phone_country" value="{$smarty.post.phone_country}" onkeypress='validate(event)' />
        <input class="phone" type="text" id="form_phone_area" name="phone_area" value="{$smarty.post.phone_area}" onkeypress='validate(event)' />
        <input type="text" id="form_phone" name="phone" value="{$smarty.post.phone}" onkeypress='validate(event)' />
	...
	<div class="row" id="form_contact_notes">
    		...
    		<textarea id="form_contact_notes" name="notes" rows="6" cols="50" placeholder="Notas personales sobre el cliente">{$smarty.post.notes}</textarea>
