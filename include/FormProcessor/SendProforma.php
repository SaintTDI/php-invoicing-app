<?php
    class FormProcessor_SendProforma extends FormProcessor
    {
        protected $db = null;
        public $user = null;
		public $mypdf1 = null;
        ...
        public function process(Zend_Controller_Request_Abstract $request)
        {
			...
			// ends new stuff
			$proforma_id = $this->sanitize($request->getPost('id'));
			$mypdf1 = $this->mypdf1;
			$mypdf = $mypdf1.'.pdf';
			$myjpg = $mypdf1.'.png';
            ...
            // if no errors have occurred, send email
             if (!$this->_validateOnly && !$this->hasError()) {          		
				...
				$body .='<tr><td colspan="3" style="font-family:Helvetica Neue, Helvetica, Arial, Geneva, sans-serif; padding: 8px; margin:5px; vertical-align: top; text-align: justify;">Le enviamos adjunta la factura pro-forma No ' . $doc_number . ' por importe de <strong>' . $amount . '</strong></td></tr>' ."\r\n";
				...
				$body .='<tr><td colspan="3" bgcolor="#0bacd6" style="font-family:Helvetica Neue, Helvetica, Arial, Geneva, sans-serif; padding: 8px; margin:5px; vertical-align: top; text-align: justify;"><a href="' . $croute . '/documents/proformas/pdf/'. $mypdf . '" target="_blank" style="color: #fff; font-weight:600; text-decoration: none; text-align:center;"><div style="height:100%;width:100%"> Factura pro-forma No ' . $doc_number . '</div></a></td></tr>' . "\r\n";
				...
				// ends new stuff
            		...
            		$subject =$company . ' te ha enviado una Factura Pro-Forma';

            		$mail = new Zend_Mail('UTF-8'); 
					
				// attach pdf and set MIME type, encoding and disposition data to handle PDF files
				$pdf1_ = $croute . '/documents/proformas/pdf/'. $mypdf;
