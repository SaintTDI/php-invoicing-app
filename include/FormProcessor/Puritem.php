<?php
    class FormProcessor_Puritem extends FormProcessor
    {
        protected $db = null;
        public $user = null;
		public $item = null;
        ...
        public function __construct($db, $user_id, $item_id = 0)
        {
            ...
            $this->item = new DatabaseObject_Puritem($db);
            $this->item->loadForUser($this->user->getId(),
									 			$item_id);
			$this->item_id = $item_id;
			$this->id_ = $this->user->getId();									
        ...
        public function process(Zend_Controller_Request_Abstract $request)
        {
			...
			if (!$this->product_id){
				$this->product_id = 0;
            		$this->product = new DatabaseObject_Product($this->db);
            		$this->product->loadForUser($this->id_, $this->product_id);
			}
