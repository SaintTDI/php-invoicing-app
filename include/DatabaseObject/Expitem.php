<?php
    class DatabaseObject_Expitem extends DatabaseObject
    {
    		public $images = array();
        ...
        {
            parent::__construct($db, 'tbl_expitem', 'item_id');
            ...
            $this->profile = new Profile_Expitem($db);
        }
        ...
        public static function GetAllItems($db, $options = array())
        {
            ...
            foreach ($items as $item_id => $item) {
                if (array_key_exists($item_id, $profiles)
                        && $profiles[$item_id] instanceof Profile_Expitem) {
     	...
     	public static function GetAllExpenseItems($db, $options = array())
        {
            // initialize the options
            $defaults = array(
                'user_id' => 0,
       ...
       public static function GetItemsId($db, $options = array())
        {
            ...
            $select = $db->select();
			$select->from('tbl_expitem', 'item_id');
			$select->where('user_id = ?', $options['user_id']);
			$select->where('document_type = ?', $options['document_type']);
	     ...
	     public function deleteItemProfile($db, $id)
        {
			...
			if(!empty($_ids)){
           		$where = $db->quoteInto('item_id in (?)', $_ids);
            		$db->delete('tbl_expitem_profile', $where);
			}
        }
        ...
        private static function _GetBaseQuery($db, $options)
        {
           ...
           $defaults = array(
            ...
            $select = $db->select();
            $select->from(array('p' => 'tbl_expitem'), array());
