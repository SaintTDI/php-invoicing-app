<?php
    class DatabaseObject_Ccompany extends DatabaseObject
    {
    		public $images = array();
        ...
        {
            parent::__construct($db, 'tbl_ccompany', 'company_id');
			
			$this->add('company_id');
            $this->add('user_id');
			$this->add('contact_id');
			$this->add('comp_doc_type');
			$this->add('ts_created', time(), self::TYPE_TIMESTAMP);

            $this->profile = new Profile_Ccompany($db);
        }
       ...
       public static function GetCompaniesCount($db, $options)
        ...
        public static function GetAllCompanies($db, $options = array())
        {
            ...
            foreach ($companies as $company_id => $company) {
                if (array_key_exists($company_id, $profiles)
                        && $profiles[$company_id] instanceof Profile_Ccompany) {
        ...
        public static function GetCompanies($db, $options = array())
        {
			...
			$select->from('tbl_ccompany', '*');
			$select->where('user_id = ?', $options['user_id']);
			$select->where('comp_doc_type = ?', $options['comp_doc_type']);
			$select->where('contact_id = ?', $options['contact_id']);
	     ...
	     public function deleteCompanyProfile($db, $id)
        {
			...
			if(!empty($_ids)){
           		$where = $db->quoteInto('company_id in (?)', $_ids);
            		$db->delete('tbl_ccompany_profile', $where);
			}
        }
        ...
        private static function _GetBaseQuery($db, $options)
        {
           ...
           $defaults = array(
            ...
            $select = $db->select();
            $select->from(array('p' => 'tbl_ccompany'), array());
