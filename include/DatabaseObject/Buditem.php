<?php
    class DatabaseObject_Buditem extends DatabaseObject
    {
    		public $images = array();
        ...
        {
            parent::__construct($db, 'tbl_buditem', 'item_id');
            ...
            $this->profile = new Profile_Buditem($db);
        }
        ...
        public static function GetAllItems($db, $options = array())
        {
            ...
            foreach ($items as $item_id => $item) {
                if (array_key_exists($item_id, $profiles)
                        && $profiles[$item_id] instanceof Profile_Buditem) {
       ...
       public static function GetItemsId($db, $options = array())
        {
            ...
            $select = $db->select();
			$select->from('tbl_buditem', 'item_id');
			$select->where('user_id = ?', $options['user_id']);
     ...
     public static function GetAllBudgetItems($db, $options = array())
        {
            // initialize the options
            $defaults = array(
                'user_id' => 0,
	     ...
	     public function deleteItemProfile($db, $id)
        {
			...
			if(!empty($_ids)){
           		$where = $db->quoteInto('item_id in (?)', $_ids);
            		$db->delete('tbl_buditem_profile', $where);
			}
        }
        ...
        private static function _GetBaseQuery($db, $options)
        {
           ...
           $defaults = array(
            ...
            $select = $db->select();
            $select->from(array('p' => 'tbl_buditem'), array());
