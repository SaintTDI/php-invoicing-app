<?php
    class DatabaseObject_Item extends DatabaseObject
    {
    		public $images = array();
        ...
        {
            parent::__construct($db, 'tbl_item', 'item_id');
            ...
            $this->profile = new Profile_Item($db);
        }
       ...
       public static function GetItemsCount($db, $options)
        ...
        public static function GetAllItems($db, $options = array())
        {
            ...
            foreach ($items as $item_id => $item) {
                if (array_key_exists($item_id, $profiles)
                        && $profiles[$item_id] instanceof Profile_Item) {
        ...
        public static function GetAllInvoiceItems($db, $options = array())
        {
            // initialize the options
            $defaults = array(
                'user_id' => 0,
       ...
       public static function GetItemsId($db, $options = array())
	     ...
	     public function deleteItemProfile($db, $id)
        {
			...
			if(!empty($_ids)){
           		$where = $db->quoteInto('item_id in (?)', $_ids);
            		$db->delete('tbl_item_profile', $where);
			}
        }
        ...
        private static function _GetBaseQuery($db, $options)
        {
           ...
           $defaults = array(
            ...
            $select = $db->select();
            $select->from(array('p' => 'tbl_item'), array());
