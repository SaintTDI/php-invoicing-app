<?php
    class DatabaseObject_PurchaseCompany extends DatabaseObject
    {
    		public $images = array();
        ...
        {
            parent::__construct($db, 'tbl_purchase_company', 'company_id');
            ...
            $this->profile = new Profile_PurchaseCompany($db);
        }
       ...
       public static function GetCompaniesCount($db, $options)
        ...
        public static function GetAllCompanies($db, $options = array())
        {
            ...
            $select = $db->select();
			$select->from('tbl_purchase_company', '*');
	     ...
	     public function deleteCompany($db, $id)
        {
			...
			if(!empty($_ids)){
           		$where = $db->quoteInto('company_id in (?)', $_ids);
            		$db->delete('tbl_purchase_company', $where);	
			}
        }
	     ...
	     public function deleteCompanyProfile($db, $id)
        {
			...
			if(!empty($_ids)){
           		$where = $db->quoteInto('company_id in (?)', $_ids);
            		$db->delete('tbl_purchase_company_profile', $where);
			}
        }
        ...
        private static function _GetBaseQuery($db, $options)
        {
           ...
           $defaults = array(
            ...
            $select = $db->select();
            $select->from(array('p' => 'tbl_purchase_company'), array());
