...
require_once $droute . '/gantt/calendar.php'; 
...
class Gantti {
  ...
  function render() {
	...
	if ($month->name() == 'January'){
    	...
    	elseif ($month->name() == 'February'){
    		$month_ = 'Febrero';
    	}
    	elseif ($month->name() == 'March'){
    		$month_ = 'Marzo';
    	}
    	elseif ($month->name() == 'April'){
    		$month_ = 'Abril';
    	}
    	elseif ($month->name() == 'May'){
    		$month_ = 'Mayo';
    	}
    	elseif ($month->name() == 'June'){
    		$month_ = 'Junio';
    	}
    elseif ($month->name() == 'July'){
    		$month_ = 'Julio';
    	}
    	elseif ($month->name() == 'August'){
    		$month_ = 'Agosto';
    	}
    	elseif ($month->name() == 'September'){
    		$month_ = 'Septiembre';
    	}
    	elseif ($month->name() == 'October'){
    		$month_ = 'Octubre';
    	}
    	elseif ($month->name() == 'November'){
    		$month_ = 'Noviembre';
    	}
	else {
    		$month_ = 'Diciembre';
    	}
      $html[] = '<li class="gantt-month" style="width: ' . ($this->options['cellwidth'] * $month->countDays()) . 'px"><strong ' . $cellstyle . '>' . $month_ . '</strong></li>';
    }                      
    $html[] = '</ul>';    
    ...
    foreach($this->blocks as $i => $block) {
	  ...
	  $days_  = number_format((float)$days,0,'.','');
      $html[] = '<span class="gantt-block' . $class . '" style="left: ' . $left . 'px; width: ' . $width . 'px; height: ' . $height . 'px"><strong class="gantt-block-label">' .  $days_ . ' D&iacute;as' . '</strong></span>';
      $html[] = '</li>';
