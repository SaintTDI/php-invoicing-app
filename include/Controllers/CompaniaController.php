<?php
    class CompaniaController extends CustomControllerAction
    {
    		protected $user = null;
        ...
        {
            parent::init();
            $this->breadcrumbs->addStep('Compania', $this->getUrl(null, 'compania'));
        ...
        public function editarcompaniaAction()
        {
			...
			$address_id = (int) $this->getRequest()->getQuery('id2');
			$cont_id = 0;
			$company_type = '';	
			...
			foreach ($companieslist as $company) {
				...
				foreach ($addresses as $address) {
				  if ($a < 1) {
					$street_[$i] = $address->profile->street;
			...
			$fp = new FormProcessor_Ccompany($this->db,
            ...
            if ($request->getPost('edit')) {
				...
				if ($fp->process($request)){
					//$address->save();
					$company->save();
                    $url = $this->getUrl('editarcompania') . '?id=' . $fp->company->getId() . '&id2=' . $fp->address->getId() . '&submitted=true';
                    $this->_redirect($url);
				}
            }
        }
		...
		public function agregarcompaniaAction()
        {
			...
			$company_id = (int) $this->getRequest()->getQuery('id');
			$address_id = (int) $this->getRequest()->getQuery('id2');
			$cont_id = (int) $this->getRequest()->getQuery('contact_id');
			$company_type = (int) $this->getRequest()->getQuery('comp_doc_type');
            ...
            if ($request->getPost('add')) {
            	//$this->messenger->addMessage('Compa&ntilde;&iacute;a aregada con exito');
                if ($fp->process($request)) {
                    $url = $this->getUrl('editarcompania') . '?id=' . $fp->company->getId() . '&id2=' . $fp->address->getId() . '&command=cerrar';
                    $this->_redirect($url);
				}
            }
       ...
       public function companiasAction()
        ...
        public function companiacompletaAction()
        {
        		// load the user record based on the stored user ID
            $user = new DatabaseObject_User($this->db);
            $user->load(Zend_Auth::getInstance()->getIdentity()->user_id);

            $this->breadcrumbs->addStep('Detalles de su Compa&ntilde;&iacute;a',
                                        $this->getUrl('direcciones'));
            $this->breadcrumbs->addStep('Compa&ntilde;&iacute;a actualizada');
            $this->view->user = $user;
